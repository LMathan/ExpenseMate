...
class ReportService {
  ...
  Future<File> generatePdfReport({DateTime? selectedMonth}) async {
    ...
    final file = File(
      '${outputDir.path}/ExpenseMate_Report_${DateTime.now().millisecondsSinceEpoch}.pdf',
    );
  ...
  Future<File> generateExcelReport({DateTime? selectedMonth}) async {
    ...
    final outputDir = await getTemporaryDirectory();
    final file = File(
      '${outputDir.path}/ExpenseMate_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx',
    );
  ...
  Future<File> generateCsvReport({DateTime? selectedMonth}) async {
    ...
    final outputDir = await getTemporaryDirectory();
    final file = File(
      '${outputDir.path}/ExpenseMate_Report_${DateTime.now().millisecondsSinceEpoch}.csv',
    );
  ...
  Future<void> shareReport(
    ...
    await Share.shareXFiles(
      [xFile],
      text: 'Here is my financial statement generated by ExpenseMate.',
      subject: subject,
    );
  }
}
